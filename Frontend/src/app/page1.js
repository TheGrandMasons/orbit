...
export default function SolarSystemScene() {
  ...
  useEffect(() => {
    ...
    const pointLight = new THREE.PointLight(0xFFF5E1, 10, 10000, 0.5);
  ...
  function createCelestialBody(data, parentBody = null) {
    ...
    if (data.name === "Earth") {
      ...
      planet.rotation.x = data.rotation_axes * Math.PI / 180;
      planet.rotation.y = -data.rotation_axes * Math.PI / 180;
    }else {
      ...
      if (data.ring == true) {
          ...
          const ringMaterial = new THREE.MeshBasicMaterial({
            color: 0xFFFFFF,
            map: textureLoader.load(data.ringMat),
            side:THREE.DoubleSide,
            transparent: true,
            opacity: 0.8
          });
          ringMaterial.wrapS = THREE.RepeatWrapping;
          ringMaterial.wrapT = THREE.RepeatWrapping;
          const ring = new THREE.Mesh(geometry, ringMaterial);
          ring.rotation.x = Math.PI / 6.6;
          planet.add(ring);
