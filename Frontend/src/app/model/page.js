...
export default function SolarSystemScene() {
  ...
  function addSkybox() {
    ...
    console.log("bg : ", `${texturePath}/imgs/stars.jpg`);
    ...
    if (data.name === "Earth") {
      ...
      if (data.ring == true) {
        ...
        const ring_material = data.mat
          ? new THREE.MeshPhongMaterial({
              map: textureLoader.load(`${texturePath}${data.ringMat}`),
    ...
    bodiesRef.current.forEach((bodyData) => {
      if (bodyData.label) {
        if (hoveredBody && hoveredBody.name === bodyData.name) {
          ...
          bodyData.label.style.color =
            bodyData.color || "rgba(255, 255, 255, 0.7)";
          if (
            bodyData.orbit &&
            bodyData.name !== selectedBodyRef.current?.name
          ) {
    ...
    if (intersects.length > 0) {
      ...
      for (const labelElement of labelElements) {
        ...
        if (
          ...
          const clickedBody = bodiesRef.current.find(
            (body) => body.name === bodyName
          );
          if (clickedBody && !clickedBody.parent) {
            handleBodySelect(clickedBody);
            return;
    ...
    <div>
      ...
      >
        {isMenuOpen ? (
          ...
          <img
            src={`${texturePath}/assets/settings.svg`}
